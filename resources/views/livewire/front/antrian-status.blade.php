<div wire:poll.3s wire:key="antrian-status" id="antrian-status-root">
    <div class="bg-gradient-to-r from-black to-gray-900 rounded-2xl p-8 text-white">
        <div class="bg-gradient-to-r from-black to-gray-900 rounded-2xl p-8 text-white mt-10">
            ...
            <div class="grid md:grid-cols-3 gap-6">
                @foreach($kapsterStats as $k)
                    <div
                        class="bg-gray-900 rounded-xl p-5 card-hover border-2 border-transparent hover:border-accent text-center">
                        <div class="flex flex-col items-center mb-3">
                            @if($k['foto'])
                                <img src="{{ asset('storage/' . $k['foto']) }}" alt="{{ $k['nama'] }}"
                                    class="h-16 w-16 rounded-full object-cover mb-2">
                            @else
                                <i class="fas fa-user text-4xl text-white mb-2"></i>
                            @endif
                            <span class="text-white font-semibold text-lg">{{ $k['nama'] }}</span>
                        </div>
                        <div class="flex justify-center gap-3 text-sm">
                            <span class="bg-yellow-600 px-3 py-1 rounded-full">Menunggu: <b>{{ $k['menunggu'] }}</b></span>
                            <span class="bg-blue-600 px-3 py-1 rounded-full">Proses: <b>{{ $k['proses'] }}</b></span>
                            <span class="bg-green-600 px-3 py-1 rounded-full">Selesai: <b>{{ $k['selesai'] }}</b></span>
                        </div>
                    </div>
                @endforeach
            </div>
